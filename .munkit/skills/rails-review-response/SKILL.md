...
| **Correct + Suggestion** | Real improvement, not blocking | Fix in this PR or record a follow-up in `.munkit/` |
| **Correct + Nice to have** | Style, minor optimisation | Optional — acknowledge explicitly |
...
preloaded at line 42 via `includes(:tags)`. Adding another `eager_load`
...
5. **Test** each fix individually — run the relevant test after each change (`bin/rails test test/<path>`).
6. **Verify** no regressions — run `bin/rails test` before requesting re-review.
