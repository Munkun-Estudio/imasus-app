...
Use this skill when the task is to review or improve the structure of a Rails application.

**Core principle:** Prioritise boundary problems over style. Prefer simple objects and explicit flow over hidden behaviour.
...
- Helpers or presenters leaking domain behaviour
...
```
**Severity:** High
**Affected file:** app/controllers/log_entries_controller.rb — LogEntriesController#create
**Risk:** Controller runs a 5-step domain workflow. Partial state on failure; untestable without HTTP.
**Improvement:** Extract to LogEntries::CreateEntry. Controller handles response/redirect only.
```
...
Fix: keep only `AuditLog.create!` in the callback; move Slack and mailer to an explicit service call at the call site.
...
| Controller action > ~15 lines | Extract to a service — controller coordinates, not implements |

## Output Style

**Begin with entry points.** Open the review by identifying the application's entry points (controllers, jobs, public surface) before listing findings. Then write findings ordered by review area — boundary problems first, then model/callback issues, then concerns/helpers.
