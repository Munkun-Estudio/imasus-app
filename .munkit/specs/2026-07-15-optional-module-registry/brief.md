...
- [x] Each supported module declares a stable key, configured label, route/helper
  metadata, content source, bookmark capability, and availability state.
- [x] Navigation, home/resources surfaces, search or linking surfaces, and
  bookmarks derive module choices from the registry rather than hardcoded lists.
- [x] Disabled modules are absent from the UI and their direct endpoints fail
  safely without affecting core workshop/project workflows.
- [x] Module combinations are validated, including dependencies between modules
  or content field types.
- [x] The all-enabled IMASUS profile and a documented minimal profile pass request,
  authorization, and system-level tests.
- [x] Adding a supported module instance requires configuration and content, not
