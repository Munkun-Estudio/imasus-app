...
## 2026-05-19: admin-facilitator-management: facilitator-centric admin surface

Workshop-facilitator assignments are managed from the per-facilitator admin page (/admin/facilitators/:id) rather than from a per-workshop admin surface. Reasons: (a) the existing "Manage facilitators" entry point on the admin home already frames the mental model around the person, not the workshop; (b) the dataset is small (a handful of facilitators with one or two workshops each), so facilitator-centric scales fine; (c) a workshop-side view can be added later if it becomes useful without rework, but is not needed to unblock the bug. The implementation uses a small Admin::FacilitatorWorkshopAssignmentsController nested under admin/facilitators that creates/destroys WorkshopParticipation rows directly. Rejected: adding "Manage facilitators" to /workshops/:slug/edit (admin-only). That would put admin actions on a page that facilitators also use and would duplicate the surface area. Revisit if multi-workshop facilitators become common and "who manages this workshop?" becomes a frequently-asked question.
