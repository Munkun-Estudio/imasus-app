...
- Production uploads use a public Tigris bucket through Active Storage's S3-compatible `amazon` service. Rails resolves attachments as redirects and public pages therefore serve object URLs directly from Tigris, rather than proxying bytes through the app. Tigris secrets are set by `flyctl storage create`; the app accepts Fly's `BUCKET_NAME` and AWS-style `AWS_S3_BUCKET`.
