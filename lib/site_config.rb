...
class SiteConfig
  ...
  Content = Data.define(:root, :workshops, :library, :guides, :prompts, :glossary)
  ...
  class Builder
    ...
    CONTENT_KEYS = %w[root workshops library guides prompts glossary].freeze
    ...
    def build_content(config)
      ...
      workshops = file!(string!(values, "workshops", "content"), "content.workshops")
      ...
      { "workshops" => workshops, "library" => library, "guides" => guides,
        "prompts" => prompts, "glossary" => glossary }.each do |key, path|
        unless inside?(path, content_root)
          raise Error, "content.#{key} must be inside content.root"
        end
      end

      Content.new(root: content_root.freeze, workshops: workshops.freeze,
                  library: library.freeze, guides: guides.freeze,
    ...
    def file!(value, path)
      ...
      resolved = @root.join(relative).cleanpath
      raise Error, "#{path} must stay inside the application root" unless inside?(resolved, @root)
      raise Error, "#{path} file does not exist: #{resolved}" unless resolved.file?
